...
from loglensx import LogParser, LogAnalyzer, LogExporter
...
def export_filtered_logs(analyzer):
    """Demonstrate exporting filtered logs."""
    print("Exporting Logs:")
    print("-" * 40)

    errors = analyzer.filter_logs(level="ERROR", limit=100)
    os.makedirs("exports", exist_ok=True)
    LogExporter.export(errors, format="csv", output_path="exports/errors.csv")
    LogExporter.export(errors, format="ndjson", output_path="exports/errors.ndjson")

    print(f"✓ Exported {len(errors)} errors to exports/errors.csv")
    print(f"✓ Exported {len(errors)} errors to exports/errors.ndjson")
    print()
